...
Recover by what the journal holds, never by when the run died. Resume (stop, edit the persisted script, relaunch with scriptPath plus resumeFromRunId) is worth choosing only when the completed prefix is unbroken -- no failed attempt anywhere before or among the calls you need replayed; a harvest recovers everything else: the journal's result events carry each completed call's full structured return value, so a few lines of code recover every finished phase and hand it to a continuation workflow that spawns only the missing work. One trap: a journal result carries only the cache key and the agent id, NEVER the label, so identity comes from the per-agent transcript, whose first user message is the prompt -- parse the distinguishing fields you embedded there (the item id, the lens name), and classify unlabeled results by their schema shape, which is unambiguous when each phase has a distinct schema. Author for this harvest in advance: embed a stable identity marker in every fan-out prompt, because a result you cannot re-key is a result you cannot reuse. Where a completed call has no journal result, its per-agent transcript holds the value as the arguments of its last structured-output tool call -- skip transcripts without one (the harness's internal stall retries leave orphan transcripts). Use transcripts to diagnose how an agent died; use the journal to recover what it produced.
...
Budget a resume at its worst case, which is a full re-run. Replay is prefix-shaped: completion order inside a concurrent stage is nondeterministic, so the cache can stop matching at the first fan-out even under a byte-identical script, and a failed attempt is a permanent cache hole -- its successful retry lives under a different cache key and does not fill it, so every later resume re-pays for the failure; one real interruption whose first fan-out contained nine scattered failures cut replay to nine calls out of roughly one hundred seventy-five completed. Estimate before relaunching, not after: count the journal's result events, find the EARLIEST failed attempt among them, and price everything downstream of that failure as re-bought -- a failure inside the first fan-out therefore costs the whole run, and one in a later phase costs every phase after it. Whenever the journal holds substantially more than the replay will recover, harvest into a continuation instead of resuming. Confirm what actually replayed (a started event on a key that already carries a result is a call being paid for twice), and detect re-execution from new transcripts appearing, never from the progress counter, whose denominator differs from the original run's.
