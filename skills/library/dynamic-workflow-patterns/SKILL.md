...
**Two tiers, never more.** The routing question is binary -- is this unit's output judgment-bearing or mechanical -- so the menu is binary too, which keeps every call decidable at a glance and the cost of a fan-out predictable. Both extremes are off the menu, and the closure is categorical rather than economic: sonnet is the floor, opus is the ceiling, and a workflow that routes outside them is misrouted no matter how the unit is shaped. The cost argument is secondary color, not the load-bearing reason -- fable multiplies a frontier price across every unit of a fan-out, and haiku's savings are not worth a wrong label or a missed finding in a stage everything downstream depends on -- so neither rationale is an exception clause that a differently-shaped unit could satisfy.
...
Treat shipped scripts as templates, never as scripts to run verbatim, and say so in the shipping skill's prose: adapt file paths, rule lists, rubrics, and model choices to the task at hand before running, because verbatim reuse forfeits the tailor-made advantage that makes dynamic workflows outperform static harnesses, and frozen scripts rot as tasks drift. Adapting model choices means re-deciding which of the two permitted tiers each role belongs on -- sonnet or opus, explicitly named on every call -- never widening the menu, because a template adapted by a reader who never opened the routing section is exactly how a third tier gets reintroduced.
