...
The tier is a capability dial, not a rescue: when the failure is mechanical instead -- skipped items, exhausted exploration, an agent that never returned -- fix the unit's scope and budget (next section), because a higher tier does not rescue an oversized unit, it only burns longer before dying.
...
Two shapes of loss reach a harvest, and a guard written for one does not cover the other. A call that RETURNS nothing hands the mapping a null VALUE, which the mapping dutifully wraps and passes on, so the reduce later dereferences a null FIELD inside a well-formed object. A call that THROWS never reaches the mapping at all: the rejection propagates out of the thunk, and parallel() substitutes a null ELEMENT for the object the mapping would have built -- so the element carries none of the fields the mapping promised, and the first access on it kills whatever code reduces the harvest. A single-argument .then inside a thunk, the shape the tool description's own examples demonstrate, guards the return and leaves the throw wide open. Either give that .then both handlers, or -- simpler, and preferred -- map AFTER the harvest, where filter(Boolean) has already established the shape and parallel()'s order-preserving result lets an index re-pair each survivor with its input.

```javascript
// WRONG: the mapping runs only when the agent returns, so a throw skips it entirely
const checked = await parallel(findings.map(f => () =>
  agent(verifyPrompt(f), { label: `verify:${f.id}`, model: 'sonnet', schema: VERDICT })
    .then(v => ({ ...f, verdict: v }))))
const confirmed = checked.filter(c => c.verdict.real)      // the null element has no .verdict

// RIGHT: harvest, then map the survivors; index pairing holds because order is preserved
const rawChecks = await parallel(findings.map(f => () =>
  tryAgent(verifyPrompt(f), { label: `verify:${f.id}`, model: 'sonnet', schema: VERDICT }, 2)))
...
Both mechanics, concretely: a harvest reads the journal's result events, which carry each completed call's full structured return value, so a few lines of code recover every finished phase; a resume means stopping the run, editing the persisted script, and relaunching with scriptPath plus resumeFromRunId. One trap on the harvest: a journal result carries only the cache key and the agent id, NEVER the label, so identity comes from the per-agent transcript, whose first user message is the prompt -- parse the distinguishing fields you embedded there (the item id, the lens name), and classify unlabeled results by their schema shape, which is unambiguous when each phase has a distinct schema. Author for this harvest in advance: embed a stable identity marker in every fan-out prompt, because a result you cannot re-key is a result you cannot reuse. Where a completed call has no journal result, its per-agent transcript holds the value as the arguments of its last structured-output tool call -- skip transcripts without one (the harness's internal stall retries leave orphan transcripts). Use transcripts to diagnose how an agent died; use the journal to recover what it produced.
